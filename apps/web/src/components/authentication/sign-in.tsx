...
export function SignInPage() {
  ...
  const handleGoogleSignIn = async () => {
    try {
      ...
      console.error(err);
      setError("Failed to initialize Google Sign In. Please verify OAuth setup.");
      setLoading(false);
    }
  };
        ...
        <div className="relative overflow-hidden rounded-2xl border border-border/80 bg-card/90 p-8 shadow-xl backdrop-blur-md">
          <div className="flex flex-col items-center text-center">
            ...
            <h1 className="text-2xl font-bold tracking-tight text-foreground sm:text-3xl">
              Welcome back to <span className="text-blue-600">Crew</span>
            </h1>
            <p className="mt-2 text-sm text-muted-foreground">
              Sign in with your Google account to discover your matching campus hobby groups and events.
            </p>
          </div>
          ...
          <div className="mt-8 space-y-4">
            ...
            <div className="flex items-center gap-2 pt-2 text-center justify-center text-xs text-muted-foreground">
              ...
              <span>Google OAuth only. No passwords required.</span>
            </div>
          </div>
